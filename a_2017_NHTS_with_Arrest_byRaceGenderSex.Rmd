...
```{r drivers and population correlation}
# d1 <- nhts_summarise_total_drivers(nhts_person, fields_to_summarise = "R_AGE")
# p1 <- census_summarise_pop_count(census17, fields_to_summarise = "R_AGE")
# left_join(d1, p1, by = "R_AGE") %>% pivot_longer(
#   cols = c("total_drivers", "total_population"),
#   names_to = "type",
#   values_to = "total"
# ) %>% filter(R_AGE >= 0 & R_AGE < 85) %>% ggplot(aes(x = R_AGE, y = total, color = type)) + geom_point() + theme(legend.position = "top") + scale_x_continuous(breaks = seq(0, 90, 5))
```

```{r percents comparison of race and sex}
# By race and sex
...
#   select(R_RACE, R_SEX, c(starts_with("percent"))) %>%
#   arrange(factor(R_RACE, levels  = race_factor_level)) %>%
#   pivot_longer(cols = c(starts_with("percent")),
#                values_to = c("percent_data")) %>%
#   filter(R_RACE != "Total", name != "percent_of_arrests_by_pop") %>%
#   ggplot(aes(
#     x = factor(R_RACE, levels = race_factor_level),
#     y = percent_data,
#     fill = factor(name, levels = percents_factor_levels)
#   )) +
...
#   # coord_flip() +
#   geom_text(
#     aes(label = paste0(round(percent_data, 0), "%"), color = factor(name, levels = percents_factor_levels)),
...
#     values = c(
#       "percent_of_drivers" = "#428BCA",
#       "percent_of_vmt" = "#4DB848",
#       "percent_of_crashes" = "#D50032",
#       "percent_of_arrests" = "#d6a60f"
#     )
#   ) + #for geom_text
...
#       "percent_of_arrests" = "#d6a60f"
#     )
#     # labels = c("Crashes", "Drivers", "VMT")
#   ) +
#   labs(
#     title = "<span style='font-size:15pt'>Share of
#     <span style='color:#428BCA;'>**drivers**</span>,
#     <span style='color:#4DB848;'>**VMT**</span>,
#     <span style='color:#D50032;'>**crashes**</span>, and
#     <span style='color:#d6a60f;'>**OWI arrests**</span> by race and sex
#     </span>"
#   ) + facet_wrap( ~ R_SEX)
```
```{r Ratio comparison of race and sex}
...
```{r percents comparison of race and age}
# By race and sex
# persons_new %>% filter(ROLE == 'Driver') %>% get_counts_of_PMD_crash_drivers(., fields_to_summarise = c("R_RACE", "age_group_10yr")) %>% select(R_RACE, age_group_10yr, nhts_n, total_drivers, PMD, crash_count) %>% filter(!is.na(age_group_10yr)) %>% mutate(
#   total_vmt = total_drivers * PMD,
#   percent_of_crashes = round(crash_count * 100 / sum(crash_count, na.rm = TRUE), 2),
#   percent_of_drivers = round(total_drivers * 100  / sum(total_drivers, na.rm = TRUE), 2),
#   percent_of_vmt = round(PMD * 100 * total_drivers / sum(PMD * total_drivers, na.rm = TRUE), 2)
# ) %>% pivot_longer(cols = starts_with("percent"),
#                    values_to = c("percent_data")) %>%
#   ggplot(aes(x = factor(age_group_10yr, levels = rev(age_group_10yr_factor_level)), y = percent_data, fill = name)) +
#   geom_bar(position = "dodge", stat = "identity") +
#   theme_bar_charts() +
#   theme(axis.text.x = element_blank()) +
#   coord_flip() +
#   geom_text(
#     aes(label = paste0(round(percent_data, 1), "%"), color = name),
#     position = position_dodge(width = .9),
#     hjust = -.2,
#     size = 3,
#     fontface = "bold"
...
#     title = "<span style='font-size:15pt'>Share of
#     <span style='color:#4DB848;'>**VMT**</span>,
#     <span style='color:#428BCA;'>**drivers**</span>, and
#     <span style='color:#D50032;'>**crashes**</span> by age and race
#     </span>"
#   ) + facet_wrap(~R_RACE)
```
...
```{r chart of age of driver and crash count for alc or drug susp VMT}
# persons_new %>% get_impaired_driver_flag() %>% filter(impaireddriver_flag == "Y") %>% 
# get_counts_of_PMD_crash_drivers(., fields_to_summarise = "age_group_10yr") %>%
# ggplot() + geom_bar(aes(x= age_group_10yr, y = crash_count/(PMD*total_drivers)*100000), stat = 'identity') + theme_classic() + labs(title = "drivers in a crash by 100,000 VMT - suspected Alc/Drug Driver")
```
...
```{r crash relative risk ratios}
# pmd_byage <- persons_new %>% filter(ROLE == "Driver") %>% person_summarise_crash_count(fields_to_summarise = c("age_group_10yr")) %>% na.omit() %>% filter(age_group_10yr %in% c("<18", "30-39"))

# poi <- nhts_summarise_pmd(nhts_person, fields_to_summarise = "age_group_10yr") %>% left_join(., pmd_byage, by = "age_group_10yr") %>% mutate(adult = ifelse(age_group_10yr %in%c("<18"), "N", "Y")) %>% group_by(adult) %>% na.omit() %>% count(sum_crash = sum(crash_count), pmd = sum(PMD))
```

```{r ratio}
get_relative_risk <- function(df, by_group, count, exposure ) {
  # df %>% filter(by_group == "N") %>% select(count)
  intervention_count = df[df$"adult"  == "N", count]
  intervention_exposure = df[df$"adult" == "N", exposure]
  control_count = df[poi$"adult" == "Y", count]
  control_exposure = df[df$"adult" == "Y", exposure]

  return((intervention_count/intervention_exposure) / (control_count/control_exposure))
}
# get_relative_risk(poi, by_group = "adult", count = "sum_crash", exposure = "pmd")

# (27177 / 1679881904) / (176133/51350027192) # teen over adult, adult as a referent
```
